	...
	$lookups = array(   
		...
		'tenants' => array(   
			'house' => array(
				'parent_table' => 'houses',
				'parent_pk_field' => 'id',
				'parent_caption' => '`houses`.`house_number`',
				'parent_from' => '`houses` ',
				'filterers' => array(),
				'custom_query' => 'SELECT `houses`.`id`, `houses`.`house_number` FROM `houses` WHERE status=\'vaccant\' ORDER BY 2',
		...
		'invoices' => array(   
			'tenant' => array(
				...
				'parent_pk_field' => 'id',
				'parent_caption' => 'IF(CHAR_LENGTH(`tenants`.`fullname`) || CHAR_LENGTH(`tenants`.`national_id`), CONCAT_WS(\'\', `tenants`.`fullname`, \' ID: \', `tenants`.`national_id`), \'\')',
				'parent_from' => '`tenants` LEFT JOIN `houses` as houses1 ON `houses1`.`id`=`tenants`.`house` ',
				'filterers' => array(),
				'custom_query' => '',
				'inherit_permissions' => false,
				'list_type' => 0,
			...
			'phone' => array(
				'parent_table' => 'tenants',
				'parent_pk_field' => 'id',
				'parent_caption' => '`tenants`.`phone_number`',
				'parent_from' => '`tenants` LEFT JOIN `houses` as houses1 ON `houses1`.`id`=`tenants`.`house` ',
				'filterers' => array(),
				'custom_query' => '',
				'inherit_permissions' => false,
			...
			'house' => array(
				'parent_table' => 'tenants',
				'parent_pk_field' => 'id',
				'parent_caption' => 'IF(    CHAR_LENGTH(`houses1`.`house_number`), CONCAT_WS(\'\',   `houses1`.`house_number`), \'\')',
				'parent_from' => '`tenants` LEFT JOIN `houses` as houses1 ON `houses1`.`id`=`tenants`.`house` ',
				'filterers' => array(),
				'custom_query' => '',
				'inherit_permissions' => false,
		...
		'payments' => array(   
			'tenant' => array(
				...
				'parent_pk_field' => 'id',
				'parent_caption' => 'IF(CHAR_LENGTH(`invoices`.`tenant`) || CHAR_LENGTH(`invoices`.`id`), CONCAT_WS(\'\', IF(    CHAR_LENGTH(`tenants1`.`fullname`) || CHAR_LENGTH(`tenants1`.`national_id`), CONCAT_WS(\'\',   `tenants1`.`fullname`, \' ID: \', `tenants1`.`national_id`), \'\'), \' INV ID: \', `invoices`.`id`), \'\')',
				'parent_from' => '`invoices` LEFT JOIN `tenants` as tenants1 ON `tenants1`.`id`=`invoices`.`tenant` LEFT JOIN `houses` as houses1 ON `houses1`.`id`=`tenants1`.`house` ',
				'filterers' => array(),
				'custom_query' => 'SELECT `invoices`.`id`, IF(CHAR_LENGTH(`invoices`.`tenant`) || CHAR_LENGTH(`invoices`.`id`), CONCAT_WS(\'\', IF(    CHAR_LENGTH(`tenants1`.`fullname`) || CHAR_LENGTH(`tenants1`.`national_id`), CONCAT_WS(\'\',   `tenants1`.`fullname`, \' ID: \', `tenants1`.`national_id`), \'\'), \' INV ID: \', `invoices`.`id`), \'\') FROM `invoices` LEFT JOIN `tenants` as tenants1 ON `tenants1`.`id`=`invoices`.`tenant` LEFT JOIN `houses` as houses1 ON `houses1`.`id`=`tenants1`.`house` WHERE `invoices`.status=\'new\' ORDER BY 2',
				'inherit_permissions' => false,
				'list_type' => 0,
				'not_null' => true
			),
